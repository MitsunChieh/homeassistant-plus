...
- [ ] 1.1 Write failing test — verify gateway devices are no longer excluded from device list, and `filter_non_gateway_devices` is replaced with a function that classifies devices as gateway or IntelliDose (test with 2 gateways + 3 IntelliDose)
...
- [ ] 1.3 Update `__init__.py` — pass both gateway and IntelliDose device lists for sensor entity creation per IntelliDose unit and gateway diagnostic entities; include all device IDs (gateways + IntelliDose) in telemetry polling with rate limit compliance
